...
namespace Bloodshot
{
	...
	bool FComponentManager::ContainsComponent(const uint32_t EntityUniqueID, const uint32_t ComponentTypeID)
	{
		BS_PROFILE_FUNCTION();
	...
	bool FComponentManager::ContainsEntity(const uint32_t EntityUniqueID)
	{
		return FEntityManager::Contains(EntityUniqueID);
	}
}
