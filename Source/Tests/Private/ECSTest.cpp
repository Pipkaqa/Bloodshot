...
namespace
{
	...
	static FEntityWrapper CreateAndTestEntity(const int ExpectedInstanceID)
	{
		...
		Wrapper.Destroy = Bind(&FEntity::Destroy, Entity.GetRawPtr());
		Wrapper.RemoveAllComponents = Bind(&FEntity::RemoveAllComponents, Entity.GetRawPtr());
	...
	static void SystemsTest()
	{
		...
		TReference<ISystem> System = IAssert::ExpectSuccess(FSystemManager::AddSystem<FTestSystem>).GetRawPtr();
